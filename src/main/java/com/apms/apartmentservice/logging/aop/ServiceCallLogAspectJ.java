package com.apms.apartmentservice.logging.aop;
...
import com.apms.apartmentservice.auth.exception.*;
 ...
 * and logs it into the database using {@link ServiceCallLogService}.
 * This aspect applies to all classes annotated with {@code @RestController}.
 *
 * @see ServiceCallLog
 * @see ServiceCallLogService
 */
@Aspect
@Slf4j
@Component
@RequiredArgsConstructor
public class ServiceCallLogAspectJ {
     ...
     * Builds a {@link ServiceCallLog} and stores it using {@link ServiceCallLogService}.
     *
    ...
    public void logAfterThrowing(JoinPoint joinPoint, Exception ex) {
        ...
        if (requestAttributes.isPresent()) {
            ...
            ServiceCallLog serviceCallLog = ServiceCallLog.builder()
            ...
            if (authentication != null && !(authentication instanceof AnonymousAuthenticationToken)) {
                ...
                serviceCallLog.setUserInfo(username);
            }

            try {
                serviceCallLogService.saveLogToDatabase(serviceCallLog);
            } catch (Exception e) {
     ...
     * Builds and stores a {@link ServiceCallLog} after the method returns.
     *
    ...
    public void logAfterReturning(JoinPoint joinPoint, Object result) throws IOException {
        ...
        if (requestAttributes.isPresent()) {
            ...
            ServiceCallLog logEntity = ServiceCallLog.builder()
                    .endpoint(request.getRequestURL().toString())
                    .method(request.getMethod())
                    .operation(joinPoint.getSignature().getName())
                    .build();
            ...
            try {
                serviceCallLogService.saveLogToDatabase(logEntity);
            } catch (Exception e) {
                log.error(e.getMessage(), e);
            }
    ...
    private String getHttpStatusFromException(Exception ex) {
        return switch (ex.getClass().getSimpleName()) {
            ...
            case "ApartmentNotFoundException" -> ApartmentNotFoundException.STATUS.name();
            case "ResidentNotFoundException" -> ResidentNotFoundException.STATUS.name();
            case "PaymentTypeNotFoundException" -> PaymentTypeNotFoundException.STATUS.name();
            case "PaymentNotFoundException" -> PaymentNotFoundException.STATUS.name();
